...
class Report:
    ...
    def _average_universe(self, outfname:str='protein_average.pdb') -> Universe:
        "Get the average structure of the trajectory"
        average = align.AverageStructure(self.universe, 
                                        None,
                                        select='protein',
                                        filename=os.path.join(self.out_path, outfname),
                                        ).run()
    ...
    def generate_pymol_reports(self, topology, trajectory, density_files, selection_string):
